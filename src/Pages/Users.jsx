...
function Users() {
  ...
  // const response=await axios.get("http://localhost:5000/api/purchase/trainer",{ headers:{
...
console.log(e);
  ...
  return (
    ...
    <div className=' w-[80vw]'>
       <div className='  '>
       <h1 className='  text-3xl font-semibold underline underline-offset-8  '>Personal Active Courses</h1>
      ...
      <div className='mt-4'></div>
       {data?.data?.map((value,i)=>
         ...
         <td><button onClick={()=>setData({...data,toggle1:true,index:i})} className=' hover:text-blue-700 underline  font-bold ' >Add</button></td>
         <td>{value?.days?.filter((a)=>a===format(new Date(),'yyyy-MM-dd'))?.length===1?<button onClick={()=>setData({...data,toggle2:true,index:i})} className=' hover:text-blue-700 underline  font-bold ' >
          Add</button>:""}</td>
        {data?.index===i&&<RemarksPopup data={data} value={value} setToggle1={setData}  />}
        {data?.index===i &&<PointsPopup data={data} value={value} setToggle2={setData}/>}
         </tr>  
         </>
         )}
          </table>
